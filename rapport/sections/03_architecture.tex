...
\subsection{Génération PDF côté client}
Afin de limiter la charge sur le serveur Node.js, la génération des documents PDF est déléguée au navigateur client. L'utilisation de la bibliothèque \texttt{jspdf} au sein des composants Angular permet de convertir les plans et l'état des données en documents imprimables localement. Ce déport de charge optimise les performances de l'application tout en garantissant un export immédiat.
...
Le backend utilise Prisma comme ORM pour interagir avec PostgreSQL. Prisma fournit un client typé généré à partir du schéma de données, facilite la rédaction de requêtes et la gestion des migrations. Nous utilisons également \texttt{prisma.\$transaction()} pour réaliser des opérations atomiques (par exemple l'import transactionnel des plans d'étage), et un singleton de connexion (\texttt{back/config/prisma.js}) pour centraliser l'initialisation du client Prisma et éviter les dépassements de connexions en environnement serveur.

\subsection{Validation et Traitement des Plans JSON}
Peu importe la méthode de création utilisée côté client (éditeur en temps réel, formulaire ou upload de fichier), les plans sont importés et traités au format JSON via une route API dédiée. Le format attendu contient les métadonnées de l'étage et des listes structurées : \texttt{rooms[]}, \texttt{doors[]}, \texttt{equipments[]}, \texttt{sockets[]}.

La sécurité et la cohérence des données sont assurées par l'ORM. L'import côté backend est entièrement transactionnel : lors d'un import massif, nous utilisons la méthode \texttt{prisma.\$transaction()} pour garantir que l'opération est atomique (soit tout l'étage est enregistré, soit la transaction est annulée en cas d'erreur de format). Le backend valide également le payload et normalise les entités en base, par exemple via la création automatique des références \texttt{room\_type} ou \texttt{equipment\_type} si elles n'existent pas encore.
...
\section[Collaboration : GitHub]{Collaboration : GitHub \raisebox{-0.2\height}{\includegraphics[height=0.6cm]{img/logo_github.png}}}
